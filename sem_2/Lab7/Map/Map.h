...
class Map {
    private:
        ...
        node* root_;
        size_t size_;
        node* get_min_node(node* current_node) {
            if (size_ > 0){
                return current_node -> left_ ? get_min_node(current_node -> left_) : current_node;
            }
        }
        node* get_max_node(node* current_node) {
            if (size_ > 0){
            return current_node -> right_ ? get_max_node(current_node -> right_) : current_node;
            }
        }
        node* erase_(node* current_node, KeyType key, bool& flag) {
            ...
            if (key < current_node -> value_.first) {
                ...
                node* min_node = get_min_node(right);
                min_node -> left_ = left;
                return right;
            }
            return current_node;
    ...
    public:
        ...
        MapIterator begin() {
            return MapIterator(get_min_node(root_ -> left_));
        }
        MapIterator end() {
        ...
        void erase(KeyType key) {
            ...
            if (size_ != 0) {
                node* max_node = get_max_node(root_ -> left_);
