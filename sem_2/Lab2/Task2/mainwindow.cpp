...
void MainWindow::on_pushButton_clicked()
{
    ...
    while(getline(file,s)){
        for(int i=0;s[i];i++) {
            ...
            if(c=='[' || c==']' || c=='{' || c=='}' || c=='(' || c==')'){
                ...
                if( elementlist->head->next == nullptr && elementlist->head->is_opening==false){
                    string result="Строка - "+to_string(stroka)+", столбец - "+to_string(stolbec);
    ...
    if(elementlist->head != nullptr) {
        string result="Строка - "+to_string(elementlist->head->str)+", столбец - "+to_string(elementlist->head->stlb);
        QString qresult=QString::fromStdString(result);
